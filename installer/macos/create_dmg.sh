#!/bin/bash
# 将 Paper-Helper.app 封装为 DMG（需 macOS + hdiutil）
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
APP_SRC="${1:-$ROOT/release/Paper-Helper.app}"
...
hdiutil create -volname "$APPNAME" -srcfolder "$DMG_TMP" -ov -format UDZO "$DMG_OUT"
rm -rf "$DMG_TMP"
